...
public class User {
	...
	public double fine() {
		...
		String sql = "SELECT deadline,return_date FROM borrow,user WHERE bo_cardID=cardID and return_date > deadline and cardID="
				+ cardID;
		String sql2 = "select deadline from borrow,user where (deadline<'" + date
				+ "' and return_date is null) and cardID=" + cardID;
		ResultSet rs = null;
		ResultSet rs1 = null;
		long betweenDays = 0;
		try {
	...
	public void submitComment(int index, String remark) {
		...
		String sql = " INSERT INTO comment(com_index,comment_stu,remark,com_date) VALUES( " + index + "," + cardID
				+ ",'" + remark + "','" + date + "' )";
		try {
			sta.execute(sql);
	...
	public ResultSet getUserCommentAverage() {
		...
		String sql = "SELECT avg(score) as avg_score,count(comment_stu) as num FROM comment WHERE comment_stu="
				+ cardID;
		ResultSet rs = null;
		try {
			rs = sta.executeQuery(sql);
	...
	public ResultSet getUserComment() {
		...
		String sql = "SELECT name,bookname,remark,score FROM user,comment,category WHERE cardID=comment_stu and com_index=`INDEX` and cardID="
				+ cardID;
		ResultSet rs = null;
		try {
			rs = sta.executeQuery(sql);
	...
	public ResultSet Cui() {
		...
		double fine = 0.1;
		Date date = new Date(System.currentTimeMillis());// 系统当前日期
		String sql = "SELECT name,bo_cardID,bo_bookid,deadline,bo_date FROM borrow,user,book WHERE bo_cardID=cardID and bo_bookid=bookid and bo_bookid=bookid and 0<deadline-'"
				+ date + "'<5 and return_date is null and cardID=" + cardID;
	...
	public ResultSet showMyBook_now() {
		...
		String sql = "SELECT bookid,bookname,bo_date,deadline FROM borrow,category JOIN book ON category.`index`=book.cateindex WHERE (bo_bookid,bookname) IN (select bookid,bookname from category JOIN book ON category.`index`=book.cateindex WHERE bookid in(select bo_bookid from borrow WHERE bo_cardID="
				+ cardID + ")) and return_date is null AND bo_cardID=" + cardID;
		ResultSet rs = null;
		try {
	...
	public ResultSet showMyBook_history() {
		...
		String sql = "SELECT bookid,bookname,bo_date,return_date FROM borrow,category JOIN book ON category.`index`=book.cateindex WHERE (bo_bookid,bookname) IN (select bookid,bookname from category JOIN book ON category.`index`=book.cateindex WHERE bookid in(select bo_bookid from borrow WHERE bo_cardID=123)) and return_date is not null and bo_cardID = "
				+ cardID;
		ResultSet rs = null;
		try {
			rs = statement.executeQuery(sql);
	...
	public ResultSet showMyBook_Reminder() {
		...
		String sql = "select a.bookname,a.`index`,r.bo_date,r.return_date from category a,book b,borrow r  where DATEDIFF(r.return_date,now())<3 and DATEDIFF(r.return_date,now())>0 and  a.`index` = b.cateindex and b.bookid = r.bo_bookid and bo_cardID= "
				+ cardID;
		ResultSet rs = null;
		try {
			rs = statement.executeQuery(sql);
	...
	public void Alter_order(int Order_index) {
		...
		String sql = "insert into `order` values(" + Order_index + "," + cardID
				+ ",now(),(select date_add(now(), interval 1 day)))";
		try {
			statement.executeUpdate(sql);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public ResultSet showMyorder() {
		...
		String sql = "select a.bookname,a.`index`,b.bookingdate,b.deadline from category a,book c,`order` b,user u where a.`index`=c.cateindex and c.bookid=b.ord_bookid and b.ord_cardID=u.cardID and cardId = "
				+ cardID;
		ResultSet rs = null;
		try {
			rs = statement.executeQuery(sql);
	...
	public static void main(String args[]) {
		...
		ResultSet rs = user.searchOverdateByUser();
		try {
			while (rs.next()) {
				String bid = rs.getString("bookid");
				String cid = rs.getString("bookname");
