...
public class Administrator {
	...
	public  ResultSet searchAdmin() {
		...
		String sql = "SELECT username,password,sex,id FROM administrator WHERE id = "+id;
		ResultSet rs = null;
		try {
	...
	public void updateadmin(String username, String password,String sex,int id) {
		// 修改管理员信息
		Statement sta = null;
		try {
			sta = connection.createStatement();
		...
		String sql = "UPDATE administrator SET `username`='"+username+"', `password`='"+password+"', `sex`='"+sex+"' WHERE `id`="+id;
		try {
			sta.execute(sql);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public static ResultSet searchHisBorByBook(int bookid) {
		...
		String sql = "SELECT `index`,bookid,bookname,cardID,`name`,bo_date,deadline,return_date FROM borrow,book,`user`,category WHERE bo_bookid=bookid AND `index`=cateindex AND bo_cardID=cardID and return_date IS NULL and  bo_bookid="
		+ bookid;
		ResultSet rs = null;
		try {
		rs = sta.executeQuery(sql);
	...
	public static void main(String[] args) {
		...
		// 测试查询当前借阅记录
		/* ResultSet rs=admin.searchAdmin();
		 try {
		 while(rs.next()){
