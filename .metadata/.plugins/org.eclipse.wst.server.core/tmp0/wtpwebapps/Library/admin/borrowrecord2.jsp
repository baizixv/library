...
<html lang="en">
<head>
<%@page import="java.sql.Date" import="com.entity.*"
	import="java.sql.ResultSet" import="java.sql.Statement"
	import="java.sql.SQLException" import="com.test.LibConnection"%>
<%!ResultSet borrow = null;
	String returndate;
	String state;
	String bookid;
	String cardid;%>
<%
	borrow = (ResultSet) session.getAttribute("SearchResult");
	if (borrow != null) {
		borrow.beforeFirst();
	}
%>
...
<link rel="stylesheet" href="/Library/assets/css/bootstrap-duallistbox.min.css" />
<link rel="stylesheet" href="/Library/assets/css/bootstrap-multiselect.min.css" />
							...
							<div class="row">
								<div class="col-xs-12">
									<table id="simple-table"
										...
										<tbody>
											<%
												//String[] BI=new String[30],CI = new String[30];
												//int i=0;
												//int[] j =new int[30];
												while (borrow.next()) {
											%>
											...
											<tr>
												<td class="center"><label class="pos-rel"><%=borrow.getString("index")%><span
														class="lbl"></span> </label></td>

												<td>
													<%
														bookid = borrow.getString("bookid");
													//	BI[i] = bookid; 
													%> <a href="#"><%=bookid%></a>
												</td>
												<td><%=borrow.getString("bookname")%></td>
												<td class="hidden-480"><%=borrow.getString("name")%></td>
												<td>
													<%
														cardid = borrow.getString("cardID");
													//	CI[i] = cardid; 
													//	j[i]=i;
													//	i++;
													%> <%=cardid%></td>

												<td><a href="#"><%=borrow.getString("bo_date")%></a></td>
												
												<td>
												<%returndate=borrow.getString("return_date");
												if(returndate==null){
													returndate="未归还";
												}
												%><%=returndate %>
...
<![endif]-->
		...
		<script src="/Library/assets/js/jquery.bootstrap-duallistbox.min.js"></script>
		<script src="/Library/assets/js/jquery.raty.min.js"></script>
		<script src="/Library/assets/js/bootstrap-multiselect.min.js"></script>
		<script src="/Library/assets/js/select2.min.js"></script>
		<script src="/Library/assets/js/typeahead.jquery.min.js"></script>
